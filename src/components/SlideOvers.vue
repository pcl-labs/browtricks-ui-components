	...
	<TransitionRoot as="template" :show="openDialog"  >
		<Dialog as="div" class="relative z-30" @click="$emit('closeModal')">
			<TransitionChild as="template" enter="ease-in-out duration-500" enter-from="opacity-0" enter-to="opacity-100" leave="ease-in-out duration-500" leave-from="opacity-100" leave-to="opacity-0">
				<div class="fixed inset-0 bg-black/70 transition ease-in-out duration-300" /> </TransitionChild>
					...
					<div class="pointer-events-none fixed inset-y-0 right-0 flex max-w-full pl-5">
						<TransitionChild as="template" 
							...
							<DialogPanel class="pointer-events-auto w-screen max-w-md pl-10">
									...
									<button type="button" class="hover:text-gray-500 focus:outline-none outline-none cursor-pointer absolute top-2.5 -left-12 hover:bg-red text-white p-2 rounded ease-in-out duration-300" @click="$emit('closeModal')"> 
										<span class="sr-only">Close panel</span>
										<component :is="XMarkIcon" class="block h-6 w-6 text-white cursor-pointer " aria-hidden="true" />
									</button>
									<div class="relative flex-1 w-full space-y-5 px-2 py-2 max-h-full overflow-y-auto overflow-x-hidden on-hover-scroll">
										<div class="space-y-5">
											<DialogTitle class="text-lg font-semibold text-gray-800 ">Upload Media</DialogTitle>
											...
											<div class="space-y-4 hidden">
												<div class="block w-full overflow-hidden rounded-lg"> <img src="@/assets/eyelarge.png" alt="" class="object-cover aspect-square w-full"> </div>
												<div class="flex items-start justify-between">
													<div class="space-y-1">
														<h2 class="text-lg font-normal text-grey-800 leading-none">
                        								  cutie_brows
                        									</h2>
														<p class="text-lg font-normal text-grey-700 flex items-center">JPG <span class="w-2 h-2 rounded-full bg-grey-500 block mx-3"></span> Jun 4, 2022</p>
													</div>
													<a
                              ...
                              >
                                <LinkChain class="w-4 h-4" />
										...
										<div class="space-y-2">
											<Textarea  rows="4" id="project-description" name="project-description" label-text="Description" placeholder="great eyebrows" labelClasses="block xs:text-lg font-semibold text-grey-800" />
 
										</div>
										<div class="space-y-2">
											<Input id="link" type="text" label-text="Link" name="link" v-model="password" placeholder="www.google.com" labelClasses="block xs:text-lg font-semibold text-grey-800" inputClasses="border-peach-500" />
                 						 </div>
										<!-- Customer -->
										<div class=" space-y-2">
											...
											<div class="flex justify-between">
												 ...
												 <a	href="#" class="text-lg font-normal text-blue leading-none inline-flex cursor-pointer">Assign to another client</a>
												 </div>
										</div>
										<!-- Assign to client -->
										<div class="space-y-2">
										...
										<!-- Visibility -->
                     				 	<div class="space-y-2">
											<p class="block text-lg font-semibold text-grey-800 ">Visibility</p>

											<CheckboxRadio id="visible" type="radio" checked="checked" name="visibility" label-text="Visible" v-model="visibility"  />
										...
										<div class="hidden gap-x-2 pt-5">
											<Button @click="editNoteDailog = false" type="submit" class="w-auto sm:px-6 bg-peach-100" :btn-primary="true">Upload</Button>
										...
										<div class="flex flex-wrap justify-between items-strech">
											<div class="flex gap-x-2 ">
												...
												<Button class="w-auto " type="button" :btn-outline="true" :btn-primary="false">Cancel</Button>
											</div>
											<Button @click="deleteCustomerDailog = true" :btn-no-border="true" :btn-primary="false" class="text-red hover:bg-red/10 xs:px-4 gap-x-2 inline-flex items-center"> <TrashIcon class="w-4"/> Delete Team </Button>  
										</div>
									</div>
								</div>
							</DialogPanel>
