<template>
  <Listbox as="div" v-model="selected">
    <ListboxLabel class="block text-sm font-semibold text-grey-800">Respond type</ListboxLabel>
    <div class="relative mt-1">
      <ListboxButton class="relative w-full cursor-default rounded-md border border-grey-300 bg-white p-3 pr-10 font-semibold text-grey-800 text-left shadow-sm focus:border-peach-500 focus:outline-none focus:ring-1 focus:ring-peach-500 text-sm ease-in-out duration-300 outline-none">
        <span class="flex items-center">
         
          <span class="block truncate">{{ selected.name }}</span>
        </span>
        <span class="pointer-events-none absolute inset-y-0 right-0 ml-3 flex items-center pr-2">
          <Chevrondown class="h-5 w-5 text-grey-800" aria-hidden="true" />
        </span>
      </ListboxButton>
            ...
            <li :class="[active ? 'text-grey-800 bg-gray-100' : 'text-grey-800', selected ? 'text-grey-800 bg-gray-100' : 'text-grey-800']" class="relative cursor-default select-none py-2 pl-3 pr-9">
              <div class="flex items-center">
                <component
                  ...
                  aria-hidden="true"
                />
                <span :class="[selected ? 'font-semibold' : 'font-semibold', 'ml-3 block truncate font-semibold']">{{ option.name }}</span>
              </div>

              <span v-if="selected" :class="[active ? 'text-grey-800' : 'text-grey-800', 'absolute inset-y-0 right-0 flex items-center mr-4']">
...
const selected = ref(checklistOptions[1])
</script>